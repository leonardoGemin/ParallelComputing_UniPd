...
int main(int argc, char **argv) {
	...
	M_TYPE **a = malloc(SIZE * sizeof(*a));
	M_TYPE **b = malloc(SIZE * sizeof(*b));
	M_TYPE **c = malloc(SIZE * sizeof(*c));
	for (i = 0; i < SIZE; i++) {
...
//	int recvcount = (procid < (SIZE % numprocs)) ? ((SIZE / numprocs) + 1) : (SIZE / numprocs);
//	M_TYPE **recvbuf = malloc(recvcount * sizeof(*recvbuf));
//	for (i = 0; i < recvcount; i++)
//		recvbuf[i] = malloc(SIZE * sizeof(*recvbuf[i]));
...
//	int sendcount = (procid < (SIZE % numprocs)) ? ((SIZE / numprocs) + 1) : (SIZE / numprocs);
//	M_TYPE **sendbuf = malloc(sendcount * sizeof(*sendbuf));
//	for (i = 0; i < sendcount; i++)
//		sendbuf[i] = malloc(SIZE * sizeof(*sendbuf[i]));
